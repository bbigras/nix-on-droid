...
{ config, lib, pkgs, home-manager-src, ... }:
...
let
  ...
  tryEval = builtins.tryEval (home-manager-src + "/modules/modules.nix");
  ...
  extendedLib = import (home-manager-src + "/modules/lib/stdlib-extended.nix") pkgs.lib;

  hmModule = types.submoduleWith {
    specialArgs = { lib = extendedLib; };
    modules = [
      ({ name, ... }: {
        imports = import (home-manager-src + "/modules/modules.nix") {
          inherit pkgs;
          lib = extendedLib;
